...
pub struct Decorations {
    ...
    bushes: Vec<Vec3>,
    rocks: Vec<Vec3>,
    rock_appearance_ids: Vec<u32>,
    cleared_soil_vertices: Vec<u32>,
}

const ROCK_BODY_COUNT_MULTIPLIER: usize = 7;
const ROCK_DROP_MINIMUM_HEIGHT: f32 = 2.0 / ISLAND_WORLD_METRES;
const ROCK_DROP_MAXIMUM_HEIGHT: f32 = 14.0 / ISLAND_WORLD_METRES;
const ROCK_DROP_NOISE_SCALE: f32 = 72.0;
const ROCK_DROP_NOISE_OCTAVES: u8 = 4;
const ROCK_DROP_NOISE_CONTRAST: f32 = 0.65;
const ROCK_DROP_NOISE_MINIMUM_DENSITY: f32 = 0.15;
const ROCK_INITIAL_HORIZONTAL_SPEED: f32 = 1.5 / ISLAND_WORLD_METRES;
const ROCK_GRAVITY: f32 = 9.81 / ISLAND_WORLD_METRES;
const ROCK_SIMULATION_STEP: f32 = 1.0 / 60.0;
const ROCK_SIMULATION_STEPS: usize = 360;
const ROCK_CONTACT_DAMPING: f32 = 0.985;
const ROCK_RESTITUTION: f32 = 0.04;
const ROCK_SLEEP_SPEED: f32 = 0.06 / ISLAND_WORLD_METRES;
const ROCK_WAKE_SPEED: f32 = 0.20 / ISLAND_WORLD_METRES;
const ROCK_SLEEP_STEPS: u8 = 24;
const ROCK_COLLISION_GRID_DIMENSION: usize = 256;
const ROCK_POOR_SOIL_MAXIMUM_COVER: f32 = 0.18;
const ROCK_MINIMUM_SETTLED_NORMAL_Z: f32 = 0.906_307_8;
const ROCK_DROP_SOURCE_MAXIMUM_NORMAL_Z: f32 = std::f32::consts::FRAC_1_SQRT_2;
const ROCK_APPEARANCE_DOMAIN: u64 = 0xd1b5_4a32_d192_ed03;
const ROCK_DROP_NOISE_DOMAIN: u64 = 0x3c6e_f372_fe94_f82b;
const ROCK_STONE_MINIMUM_DIAMETER_METRES: f32 = 0.10;
const ROCK_STONE_MAXIMUM_DIAMETER_METRES: f32 = 0.30;
const ROCK_BOULDER_MINIMUM_DIAMETER_METRES: f32 = 0.30;
const ROCK_BOULDER_MAXIMUM_DIAMETER_METRES: f32 = 0.60;
const RIVER_POINT_SAMPLE_SPACING: f32 = 2.0 / ISLAND_WORLD_METRES;
...
impl Decorations {
    ...
    fn cleared_soil_vertices(&self) -> &[u32] {
...
impl RockCollisionGrid {
    fn new(body_capacity: usize) -> Self {
    ...
    fn rebuild(&mut self, bodies: &[RockBody]) {
        self.counts.fill(0);
        for body in bodies {
...
fn spawn_rock_bodies(seed: u64, terrain: &Terrain, target: usize) -> Vec<RockBody> {
...
fn rock_drop_face_weight(seed: u64, mesh: &Mesh, triangle: &[u32]) -> f32 {
    let [a, b, c] =
...
fn rock_drop_noise_density(noise: f32) -> f32 {
...
fn sample_triangle(a: Vec3, b: Vec3, c: Vec3, rng: &mut Rng) -> Vec3 {
    let radial = rng.unit().sqrt();
    let along = rng.unit();
    a * (1.0 - radial) + b * (radial * (1.0 - along)) + c * (radial * along)
}

fn rock_collision_radius(seed: u64, appearance_id: u32) -> f32 {
...
fn is_rock_habitat(material: Vec3) -> bool {
    material.x > 1.5 || material.y <= ROCK_POOR_SOIL_MAXIMUM_COVER
}

fn is_rock_drop_source(normal: Vec3) -> bool {
    normal.is_finite() && normal.z.abs() <= ROCK_DROP_SOURCE_MAXIMUM_NORMAL_Z
}

fn clear_loose_soil(material: &mut SurfaceMaterial, vertices: &[u32]) {
...
fn inside_decoration_bounds(point: Vec2) -> bool {
    (0.01..=0.99).contains(&point.x) && (0.01..=0.99).contains(&point.y)
}

fn simulate_rock_bodies(terrain: &Terrain, bodies: &mut [RockBody]) {
...
fn constrain_rock_to_island(body: &mut RockBody) {
...
fn resolve_rock_body_contacts(bodies: &mut [RockBody], grid: &RockCollisionGrid) {
...
fn resolve_rock_pair(first: &mut RockBody, second: &mut RockBody) {
...
mod decoration_tests {
    use super::{
        Island, IslandOptions, Mesh, ROCK_MINIMUM_SETTLED_NORMAL_Z, ROCK_POOR_SOIL_MAXIMUM_COVER,
        RockBody, SurfaceMaterial, Terrain, Vec3, clear_loose_soil, is_rock_drop_source,
        is_rock_habitat, resolve_rock_pair, resolve_rock_terrain_contact, rock_collision_radius,
        rock_drop_face_weight, rock_drop_noise_density, sample_triangle,
    };

    #[test]
    fn existing_rock_habitat_does_not_require_soil_clearing() {
    ...
    #[test]
    fn drop_sources_are_steep_faces() {
    ...
    #[test]
    fn steep_face_weight_uses_surface_area_and_noise() {
        let mesh = Mesh {
            vertices: vec![
    ...
    #[test]
    fn drop_noise_keeps_low_regions_eligible_and_strengthens_dense_regions() {
    ...
    #[test]
    fn triangle_sampling_stays_inside_the_source_face() {
    ...
    #[test]
    fn collision_radius_matches_the_unity_size_ranges_deterministically() {
    ...
    #[test]
    fn terrain_contact_lifts_a_falling_body_onto_the_surface() {
    ...
    #[test]
    fn body_contact_separates_overlapping_rocks_and_supports_the_upper_one() {
    ...
    #[test]
    fn steep_terrain_contact_does_not_become_stable_support() {
        let normal = Vec3::new(
    ...
    #[test]
    fn clearing_a_support_triangle_removes_all_three_soil_depths() {
    ...
    #[test]
    fn generated_grass_settlements_clear_the_exported_vertex_cover() {
    ...
    fn body(appearance_id: u32, centre: Vec3, radius: f32) -> RockBody {
        RockBody {
            centre,
            velocity: Vec3::ZERO,
            radius,
