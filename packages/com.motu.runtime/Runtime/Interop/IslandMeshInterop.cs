...
namespace Motu.Interop
{
    internal static class IslandMeshInterop
    {
        ...
        internal static IslandPreparedMesh CopyNavigationMeshData(MotuNative.ExportMesh source, float worldSize)
        {
            if (source.vertices.length == 0 && source.triangles.length == 0) return null;
            if (source.vertices.data == IntPtr.Zero || source.triangles.data == IntPtr.Zero
                || source.triangles.length <= 0 || source.triangles.length % 3 != 0)
                throw new InvalidOperationException("Invalid native navigation mesh.");
            var vertices = CopyVector3Array(source.vertices, true, worldSize);
            var indices = new int[source.triangles.length];
            Marshal.Copy(source.triangles.data, indices, 0, indices.Length);
            for (var i = 0; i < indices.Length; i += 3)
            {
                for (var corner = 0; corner < 3; corner++)
                    if (indices[i + corner] < 0 || indices[i + corner] >= vertices.Length)
                        throw new InvalidOperationException("Invalid native navigation triangle index.");
                (indices[i + 1], indices[i + 2]) = (indices[i + 2], indices[i + 1]);
            }
            // Navigation needs geometry and the river mask, not render normals,
            // texture UVs, or environment sidecars for this full-island copy.
            return new IslandPreparedMesh(vertices, Array.Empty<Vector3>(), indices,
                Array.Empty<Vector2>(), CopyMaterialArray(source.material), Array.Empty<Vector2>());
        }
