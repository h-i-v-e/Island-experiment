...
namespace Motu.Islands
{
    internal static class IslandSnapshotCache
    {
        ...
        internal static string CacheDirectory
        {
            get
            {
                return Path.Combine(Application.persistentDataPath, "GeneratedIslandCache");
            }
        }
        ...
        internal static string PathFor(IslandGenerationRequest request)
        {
            ...
            var requestedDirectory = request.Generation.SnapshotCacheDirectory;
            if (!string.IsNullOrWhiteSpace(requestedDirectory) && !Path.IsPathRooted(requestedDirectory))
                throw new ArgumentException("The snapshot cache directory must be absolute.");
            var directory = string.IsNullOrWhiteSpace(requestedDirectory) ? CacheDirectory : Path.GetFullPath(requestedDirectory);
        ...
        internal static IntPtr TryLoad(string path, out int status)
        {
            ...
            if (handle != IntPtr.Zero)
            {
                ...
                catch (IOException) { } // A readable cache can be mounted read-only.
        ...
        internal static bool TrySave(IntPtr handle, string path, long budgetBytes)
        {
            ...
            catch (IOException error) { Debug.LogWarning($"Motu snapshot cache unavailable: {error.Message}"); return false; }
            catch (UnauthorizedAccessException error) { Debug.LogWarning($"Motu snapshot cache unavailable: {error.Message}"); return false; }
            ...
            string protectedPath)
        {
            ...
            try
            {
                var files = new DirectoryInfo(directory).GetFiles()
                    .Where(file => IsOwnedCacheFile(file.Name)).ToArray();
        ...
        private static bool IsOwnedCacheFile(string name)
        {
            // Never evict another writer's temporary files or unrelated host files.
            if (name.Length < 65 || name[64] != '.') return false;
            for (var i = 0; i < 64; i++)
                if (!(name[i] >= '0' && name[i] <= '9') && !(name[i] >= 'a' && name[i] <= 'f')) return false;
            var extension = name.Substring(64);
            return extension == SnapshotExtension || extension == ".motumaterials";
        }
