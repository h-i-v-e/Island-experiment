...
namespace Motu.Islands
{
    ...
    public sealed partial class IslandRuntime : MonoBehaviour, IDisposable
    {
        ...
        internal void OwnResource(IDisposable resource)
        {
            RequireInstalling();
            if (resource != null) ownedResources.Add(resource);
        }
        ...
        public IIslandNavigation Navigation { get; private set; }
        internal void SetNavigation(IIslandNavigation value)
        {
            RequireInstalling();
            if (!(value is Component component) || !component.transform.IsChildOf(transform))
                throw new ArgumentException("Navigation must belong to its island runtime.");
            Navigation = value;
        }
        ...
        {
            RequireInstalling();
            // Standalone terrain has no global coastal-wave consumer.
        ...
        internal void SetDormant(bool dormant)
        {
            ...
            Navigation?.SetRegistered(!dormant);
