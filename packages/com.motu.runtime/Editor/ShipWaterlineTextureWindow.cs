...
namespace Motu.Editor
{
    public sealed class ShipWaterlineTextureWindow : EditorWindow
    {
        ...
        private void SaveAndAssign()
        {
            try
            {
                var assetPath = EditorUtility.SaveFilePanelInProject("Save Waterline Texture", "Ship Waterline", "png", "Choose where to save the generated texture.");
                if (string.IsNullOrEmpty(assetPath)) return;
                savedTexture = SaveAndAssign(shipRoot, preview, previewForward, waterlineWorldY, settings, assetPath);
        ...
        internal static Texture2D SaveAndAssign(Transform root, ShipWaterlineTextureBuilder.Result result, Vector3 forward, float worldY,
            ShipWaterlineTextureBuilder.Settings settings = null, string assetPath = null)
        {
            if (string.IsNullOrWhiteSpace(assetPath) || !assetPath.StartsWith("Assets/", StringComparison.Ordinal)
                || !assetPath.EndsWith(".png", StringComparison.OrdinalIgnoreCase))
                throw new ArgumentException("Choose a PNG destination inside the host project's Assets folder.", nameof(assetPath));
