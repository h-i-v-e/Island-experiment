...
{
    // Reeds are generated only for the finest terrain owner grid. This streamer
    // lazily uploads the same 3x3 neighborhood as terrain LOD0 and never creates a
    // GameObject for an individual clump.
    internal sealed class ReedTileStreamer : IDisposable
    {
            ...
            bool showReeds)
        {
            ...
            if (preparedTiles == null || preparedTiles.Length != TileCount)
            {
                throw new ArgumentException(
                    $"The reed owner grid must contain {TileCount} tiles.",
                    nameof(preparedTiles));
            }
            prepared = preparedTiles;
            root = new GameObject("Riverbank Reeds and Rushes (LOD 0 Only)");
            root.transform.SetParent(parent, false);
            root.SetActive(showReeds);
        }
        ...
        private StreamedMeshTile CreateTile(Vector2Int key)
        {
            ...
            return StreamedMeshTile.Create(source, root.transform, material, $"Riverbank reeds {key.x},{key.y}");
        }
