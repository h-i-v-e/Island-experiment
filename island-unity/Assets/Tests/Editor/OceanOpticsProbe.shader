Shader "Hidden/Motu/Tests/Ocean Optics"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                if (_ProbeMode < .5) return float4(MotuOceanTransmission(_ProbeDistance), MotuOceanFresnel(input.uv.x));
                if (_ProbeMode < 1.5)
                {
                    float roughness;
                ...
                if (_ProbeMode < 2.5) return MotuOceanSunSpecular(float3(0, 1, 0), normalize(_ProbeView.xyz), normalize(_ProbeLight.xyz), _SurfaceRoughness);
                return MotuOceanRefractionValidity(_ProbeDistance, 10);
            }
            ENDCG
        }
    }
