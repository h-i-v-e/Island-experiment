...
namespace Motu.Editor
{
    public static class IslandNativeValidation
    {
        ...
        {
            var settings = new IslandGenerationSettings { InitialSoilDepthMetres = 2.5f };
            var options = settings.ToNativeOptions(new IslandRiverSettings());
            if (Marshal.SizeOf<MotuNative.Options>() != 80)
                throw new InvalidOperationException("Initial soil native options layout is incorrect.");
            var handle = MotuNative.CreateMotu(17, ref options);
            if (handle == IntPtr.Zero)
                throw new InvalidOperationException("Native island generation with initial soil failed.");
            var map = IntPtr.Zero;
            try
            {
                map = MotuNative.CreateHeightMap(handle, 33);
                if (map == IntPtr.Zero)
                    throw new InvalidOperationException("Initial soil island has no height map.");
                var exported = Marshal.PtrToStructure<MotuNative.ExportHeightMapWithSeaLevel>(map);
                var heights = new float[exported.width * exported.height];
                Marshal.Copy(exported.data, heights, 0, heights.Length);
                if (heights.Length != 33 * 33 || Array.Exists(heights, height => !IsFinite(height)))
                    throw new InvalidOperationException("Initial soil island has invalid terrain heights.");
            }
            finally
            {
                if (map != IntPtr.Zero) MotuNative.ReleaseHeightMap(map);
                MotuNative.ReleaseMotu(handle);
            }
            Debug.Log("Native island generation and finite height map with 2.5 m initial soil passed.");
        }
            ...
            try
            {
                ...
                try
                {
                    var requiredTerrainProperties = new[]
                    {
                        ...
                        "_GrassPatchNoiseWorldSize", "_SandPatchNoiseWorldSize",
